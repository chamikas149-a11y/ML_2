...
st.set_page_config(page_title="Advanced BMS Monitor", layout="wide")
st.title("🛡️ Advanced Battery Thermal Management System")
st.markdown(f"**Researcher:** CHAMIKA SANKALPA | **Industry:** KIK Lanka Pvt Ltd")
...
temp_res = scaler_y.inverse_transform(prediction)[0][0]

# Add to History
new_data = pd.DataFrame({'Time': [datetime.now().strftime("%H:%M:%S")], 
                         'Voltage': [v_in], 'Current': [i_in], 'Temp': [temp_res]})
st.session_state.history = pd.concat([st.session_state.history, new_data]).tail(10)

# 6. Dashboard Layout (Columns)
...
# 7. Visualization - Temperature Gauge & Line Chart
chart_col, gauge_col = st.columns([2, 1])

with chart_col:
    st.subheader("📈 Temperature Trend (Last 10 Readings)")
